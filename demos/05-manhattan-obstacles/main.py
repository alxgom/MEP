...
- 1: Manhattan MST (Baseline / Shortest Path Heuristic)
- 2: Greedy Steiner (Iterated 1-Steiner)
- 3: Grid Pruning (Top-Down Dense Grid)
- 4: Fast Corner Kick (L-Bend targeted, deterministic)
- 5: Monte Carlo Population (Evolutionary)
- 6: Kou-Markowsky-Berman (KMB '81, guaranteed 2x approx)
- 7: Stochastic KMB (perturbed metric closure, best of 20 trials)
- 8: Anisotropic KMB (directional A*, w_y=2x, 15 trials)
- C: Clear map
...
class Visualizer:
    def __init__(self, width=1000, height=800):
        ...
        self.mode_names = {
            ...
            6: "Kou-Markowsky-Berman (KMB '81)",
            7: "Stochastic KMB (20 trials)",
            8: "Anisotropic KMB A* (w_y=2x, 15 trials)",
        }
    ...
    def _solve(self):
        ...
        elif self.mode == 6: self.solution = solver.solve_kou()
        elif self.mode == 7: self.solution = solver.solve_stochastic_kou(n_trials=20, perturbation=0.10)
        elif self.mode == 8: self.solution = solver.solve_anisotropic_kou(w_x=1.0, w_y=2.0, n_trials=15, sigma=0.3)
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.button == 1:
                        self.terminals.append(list(pos))
                        self._solve()
                    elif event.button == 3:
                        self.current_obstacle_start = pos
                elif event.type == pygame.MOUSEBUTTONUP:
                    if event.button == 3 and self.current_obstacle_start:
                        pos = pygame.mouse.get_pos()
                        obs = Obstacle(
                            self.current_obstacle_start[0],
                            self.current_obstacle_start[1],
                            pos[0], pos[1],
                        )
                    ...
                    elif event.key == pygame.K_6: self.mode = 6; self._solve()
                    elif event.key == pygame.K_7: self.mode = 7; self._solve()
                    elif event.key == pygame.K_8: self.mode = 8; self._solve()

            # Draw obstacle drag preview
            if self.current_obstacle_start:
                curr_pos = pygame.mouse.get_pos()
                x = min(self.current_obstacle_start[0], curr_pos[0])
                y = min(self.current_obstacle_start[1], curr_pos[1])
                w = abs(self.current_obstacle_start[0] - curr_pos[0])
                h = abs(self.current_obstacle_start[1] - curr_pos[1])
            ...
            for obs in self.obstacles:
                pygame.draw.rect(
                    ...
                    (obs.min_x, obs.min_y, obs.max_x - obs.min_x, obs.max_y - obs.min_y),
                )

            # Draw Hanan grid nodes
            if self.env:
                for node in self.env.nodes:
                    pygame.draw.circle(self.screen, COLOR_GRID, (int(node[0]), int(node[1])), 1)

            # Draw solution
            if self.solution and self.env:
                for u_idx, v_idx in self.solution["segments"]:
                    pygame.draw.line(
                        self.screen, COLOR_PATH,
                        self.env.nodes[u_idx], self.env.nodes[v_idx], 4,
                    )
            ...
            info = [
                f"MODE: {self.mode_names[self.mode]} (Keys 1-8)",
                "LEFT CLICK: Add Terminal | RIGHT DRAG: Obstacle",
                "C: Clear | R: Random (8 pts)",
