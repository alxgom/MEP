...
The Routing-Core Workflow mode mirrors the core placement phase at demo scale: it generates candidates from machine-room centroids, optional +/-250 mm translations along routing-frame axes, and four connector-aligned rotations. As in core's current optional-Clima generator, candidates are not prefiltered at this step. They are sorted with routing-core's final Clima machine sort: connector-to-conditioned-room-centroid distance, air-side room clearance, and refrigeration-side room clearance. It does not rerun the full route for each candidate; an invalid selected candidate remains visible as an invalid interactive result rather than changing the placement sort.
