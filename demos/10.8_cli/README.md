...
`Routing-Core Port: Simple Grid` is the corresponding NumPy port of routing-core's `_create_grid_dynamic_simple` input geometry. It uses exact supply Steiner terminals, the core 200 mm inset allowed region, inward allowed-boundary corner axes, and Cli shaft/column obstacles buffered from the active machine air-connector width with core `BUFFER_RATIO = 0.6`. It retains the core behavior of making an invalid edge very expensive rather than removing it. Machine-body blocking is applied by the demo's dynamic environment; the core connector-segment obstacle polygons are still a documented gap because their terminal exceptions need to be ported together. The existing Hannan and epsilon grids remain separate interactive alternatives because they add scaffolding, wall shifts, and recovery links not present in the simple core grid.
