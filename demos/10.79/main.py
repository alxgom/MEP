...
def get_canvas_tool_buttons():
    ...
    return [
        ...
        ("ruler", pygame.Rect(x0 + 2 * (size + gap) + reset_w + gap, y0, 58, size), "Ruler"),
        ("weights", pygame.Rect(x0 + 2 * (size + gap) + reset_w + gap + 58 + gap, y0, 72, size), "Weights"),
        ("weight_view", pygame.Rect(x0 + 2 * (size + gap) + reset_w + gap + 58 + gap + 72 + gap, y0, 58, size), "Small" if edge_weight_view_mode_idx == 0 else "Big"),
        ("diameter", pygame.Rect(x0 + 2 * (size + gap) + reset_w + gap + 58 + gap + 72 + gap + 58 + gap, y0, 54, size), "Diam"),
    ]

def handle_canvas_tool_button_click(pos):
    global route_real_diameter_width_enabled, edge_weight_view_mode_idx
...
def record_current_solution(routes, elapsed_ms, marker_label=None, marker_color=(241, 196, 15)):
    if routes:
        ...
        if marker_label:
            hist_event_markers.append((len(hist_length) - 1, marker_label, marker_color))
...
def draw_canvas_tool_controls(screen, font_small, ruler_mode):
    for action, rect, label in get_canvas_tool_buttons():
        active = (
            ...
            or (action == "weight_view" and edge_weight_heatmap_enabled)
...
def run_auto_placement():
    ...
    elif auto_placement_mode_idx == 3:
...
def main():
    ...
    pygame.display.set_caption("Integrated Auto-Placement & Ventilation Router (Demo 10.79)")
    ...
    while running:
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_F11:
                    ...
                    if routes and not status.startswith("Blocked"):
                        crossings_c = count_segment_crossings(routes)
                        record_history(routes, crossings_c, elapsed_ms)
                        hist_event_markers.append((len(hist_length) - 1, f"Src:{dwelling_source_idx}", (52, 152, 219)))
                    ...
                    if routes and not status.startswith("Blocked"):
                        crossings_c = count_segment_crossings(routes)
                        record_history(routes, crossings_c, elapsed_ms)
                        hist_event_markers.append((len(hist_length) - 1, f"Frame:{routing_frame_idx}", (230, 126, 34)))
                    ...
                    if routes and not status.startswith("Blocked"):
                        crossings_c = count_segment_crossings(routes)
                        record_history(routes, crossings_c, elapsed_ms)
                        hist_event_markers.append((len(hist_length) - 1, f"Start:{room_start_mode_idx}", (155, 89, 182)))
                    ...
                    if routes and not status.startswith("Blocked"):
                        crossings_c = count_segment_crossings(routes)
                        record_history(routes, crossings_c, elapsed_ms)
                        hist_event_markers.append((len(hist_length) - 1, f"Heur:{heuristic_mode_idx}", (241, 196, 15)))
        ...
        solver_card = pygame.Rect(15, 220, CANVAS_LEFT - 40, 210)
        ...
        lbl_selected = font_small.render(f"Selected: {selected_text[:20]}", True, COLOR_TEXT)
        screen.blit(lbl_selected, (25, 365))
        draw_min_piece_slider(screen, font_small, 25, 385, CANVAS_LEFT - 70)
        lbl_keys = font_small.render("[C] Strategy | [M] Weights | [F11] Full", True, COLOR_MUTED)
        screen.blit(lbl_keys, (25, 415))
        
        # 3. Placement Info Card
        machine_card = pygame.Rect(15, 440, CANVAS_LEFT - 40, 105)
        ...
        screen.blit(lbl_pos_title, (25, 450))
        ...
        screen.blit(lbl_scenario, (25, 470))
        ...
        screen.blit(lbl_kpi_title, (25, 565))
        ...
        screen.blit(lbl_score, (25, 665))
        
        # 5. Status Box
        status_card = pygame.Rect(15, 700, CANVAS_LEFT - 40, 210)
        ...
        screen.blit(lbl_status_title, (25, 710))
