...
`Routing-Core Port: Simple Grid` is the corresponding NumPy port of routing-core's `_create_grid_dynamic_simple` input geometry. It uses exact supply Steiner terminals, inward allowed-boundary corner axes, and only Cli shafts/columns as obstacles. It retains the core behavior of making an invalid edge very expensive rather than removing it. The existing Hannan and epsilon grids remain separate interactive alternatives because they add scaffolding, wall shifts, and recovery links not present in the simple core grid.
